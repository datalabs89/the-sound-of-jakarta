...
<body>
    ...
    <div class="header-area">
      ...
      <div class="city-summary-grid">
        ...
        <div class="city-card" id="card-JAKARTA-UTARA" onclick="filterCity('JAKARTA UTARA')">
          ...
          <div class="city-card-lead prabowo">02 Lead: 24 Vil</div>
        </div>
        ...
        <div class="city-card" id="card-JAKARTA-BARAT" onclick="filterCity('JAKARTA BARAT')">
          ...
          <div class="city-card-lead prabowo">02 Lead: 47 Vil</div>
        </div>
        ...
        <div class="city-card" id="card-JAKARTA-PUSAT" onclick="filterCity('JAKARTA PUSAT')">
          ...
          <div class="city-card-lead anies">01 Lead: 24 Vil</div>
        </div>
  ...
  <script>
    ...
    const satelliteRegions = [
      ...
      {{ name: '📍 BEKASI CITY & REGENCY', lat: -6.200, lon: 107.075, isSea: false }}
    ];

    satelliteRegions.forEach(reg => {{
      const icon = L.divIcon({{
        className: 'surrounding-region-label',
        html: `<div class="surrounding-badge ${{reg.isSea ? 'sea-badge' : ''}}">${{reg.name}}</div>`,
        iconSize: [195, 26],
        iconAnchor: [97, 13]
      }});
      L.marker([reg.lat, reg.lon], {{ icon: icon, interactive: false }}).addTo(map);
    }});
