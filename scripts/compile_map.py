...
<head>
  ...
  <style>
    ...
    * {{
    ...
    /* SUMMARY STATS BAR */
    /* SUMMARY KPI METRICS BAR & INDEPENDENT CARDS */
    .stats-bar {{
      display: grid;
    ...
    .stat-box.anies {{
      border-top: 3.5px solid var(--c-anies);
      background: #FAF3F5;
      border-color: #E6D5D9;
    }}

    .stat-box.prabowo {{
      border-top: 3.5px solid var(--c-prabowo);
      background: #F3F6FA;
      border-color: #D5E1EC;
    }}

    .stat-box.ganjar {{
      border-top: 3.5px solid var(--c-ganjar);
      background: #F7F3F6;
      border-color: #E1D6DE;
    }}

    .stat-box.battleground {{
      border-top: 3.5px solid var(--c-close);
      background: #FAF5EE;
      border-color: #ECDDC9;
    }}
    ...
    .stat-box.anies .stat-label {{ color: #7A283C; }}
    .stat-box.prabowo .stat-label {{ color: #2D557B; }}
    .stat-box.ganjar .stat-label {{ color: #5A3852; }}
    .stat-box.battleground .stat-label {{ color: #9C5516; }}
    ...
    /* Solid Grayscale Checkbox Styling (Override OS / Browser Blue) */
    .basemap-toggle-wrap input[type="checkbox"],
    .labels-toggle-wrap input[type="checkbox"],
    #chkBasemap,
    #chkHexLabels {{
    ...
    #chkBasemap:focus-visible,
    #chkHexLabels:focus-visible {{
      outline: 2px solid #111111;
      outline-offset: 1px;
    }}

    /* Solid Grayscale Range Slider Styling (Override OS / Browser Blue) */
    #basemapOpacity {{
    ...
    #basemapOpacity:focus {{
      outline: none;
    }}

    #basemapOpacity:focus-visible {{
      outline: 2px solid #111111;
      outline-offset: 2px;
    }}

    /* WebKit (Chrome, Safari, Edge, Opera) Track */
    #basemapOpacity::-webkit-slider-runnable-track {{
      width: 100%;
      height: 4px;
      cursor: pointer;
      background: #C4BCAC;
    ...
    /* WebKit Thumb */
    #basemapOpacity::-webkit-slider-thumb {{
      -webkit-appearance: none;
      appearance: none;
      height: 13px;
      width: 13px;
    ...
    #basemapOpacity::-webkit-slider-thumb:hover {{
      background: #2B2B2B;
      transform: scale(1.15);
    }}

    #basemapOpacity::-webkit-slider-thumb:active {{
      background: #000000;
      transform: scale(1.2);
    }}

    /* Firefox Track */
    #basemapOpacity::-moz-range-track {{
      width: 100%;
      height: 4px;
      cursor: pointer;
      background: #C4BCAC;
    ...
    /* Firefox Progress (before thumb) */
    #basemapOpacity::-moz-range-progress {{
      background: #111111;
      height: 4px;
      border-radius: 2px;
    }}
    ...
    /* Firefox Thumb */
    #basemapOpacity::-moz-range-thumb {{
      height: 13px;
      width: 13px;
      border-radius: 50%;
      background: #111111;
    ...
    #basemapOpacity::-moz-range-thumb:hover {{
      background: #2B2B2B;
      transform: scale(1.15);
    }}

    #basemapOpacity::-moz-range-thumb:active {{
      background: #000000;
      transform: scale(1.2);
    }}
    ...
    .custom-perimeter-icon {{
      background: transparent !important;
      border: none !important;
      overflow: visible !important;
    }}

    .custom-perimeter-icon-central {{
      background: transparent !important;
      border: none !important;
      overflow: visible !important;
    }}

    .central-marker-wrapper {{
      display: flex;
      flex-direction: column;
      align-items: center;
      width: 174px;
    ...
    .central-marker-wrapper .city-perimeter-label {{
      pointer-events: auto;
      width: 174px;
      height: 28px;
    }}

    .central-callout-line-dom {{
      width: 0px;
      border-left: 1.5px dashed #111111;
      height: 106px;
      opacity: 0.75;
      transition: height 0.1s ease-out, opacity 0.15s ease;
    }}

    .central-callout-dot-dom {{
      width: 7px;
      height: 7px;
      border-radius: 50%;
      background: #AF4D64;
    ...
    .central-marker-wrapper:hover .central-callout-line-dom {{
      border-left-width: 2.2px;
      opacity: 1;
    }}

    .central-marker-wrapper:hover .central-callout-dot-dom {{
...
<body>
        ...
        <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
          <div class="basemap-toggle-wrap" title="Toggle Mapbox city streets context, choose style and adjust opacity">
            <label style="display: flex; align-items: center; gap: 4px; cursor: pointer; user-select: none;">
              <input type="checkbox" id="chkBasemap" checked onchange="toggleBasemap(this.checked)" style="cursor: pointer; accent-color: #111111;">
              <span>Streetmap</span>
            </label>
            ...
            <input type="range" id="basemapOpacity" min="0.1" max="1.0" step="0.05" value="0.60" oninput="setBasemapOpacity(this.value)" title="Basemap Opacity" style="accent-color: #111111;">
          ...
          <div class="labels-toggle-wrap" title="Toggle village names on hex cells (Auto-appears when zooming in)">
            <label style="display: flex; align-items: center; gap: 5px; cursor: pointer; user-select: none;">
              <input type="checkbox" id="chkHexLabels" onchange="toggleHexLabels(this.checked)" style="cursor: pointer; accent-color: #111111;">
  ...
  <script>
    ...
    perimeterCityLabels.forEach(c => {{
      ...
      if (c.kota === 'JAKARTA PUSAT') {{
        const centralIcon = L.divIcon({{
          className: 'custom-perimeter-icon-central',
          html: `
            <div class="central-marker-wrapper" id="centralMarkerWrap">
              <div class="city-perimeter-label" onclick="filterCity('${{c.kota}}')">
                <span class="c-line" style="background:${{c.color}}"></span>
                <span class="c-name">${{c.name}}</span>
              </div>
              <div class="central-callout-line-dom" id="centralLineDom"></div>
              <div class="central-callout-dot-dom" id="centralDotDom"></div>
            </div>
          `,
          iconSize: [w, h],
        ...
        const m = L.marker([c.lat, c.lng], {{ icon: centralIcon, interactive: true }}).addTo(map);
    ...
    // Responsive Auto-Calibrated Height for Central Jakarta Leader Line
    function updateCentralLineHeight() {{
      const line = document.getElementById('centralLineDom');
      if (!line) return;
      const pTop = map.latLngToLayerPoint([-6.0120, 106.8421]);
      const pBottom = map.latLngToLayerPoint([-6.0964, 106.8421]);
      const h = Math.max(10, Math.round((pBottom.y - pTop.y) - 17.5));
      line.style.height = h + 'px';
    }}

    map.on('zoomend', updateCentralLineHeight);
    setTimeout(updateCentralLineHeight, 150);
    ...
    function filterCity(cityName) {{
      ...
      // Dim or restore Central Jakarta unified marker (card + callout line + dot)
      const centralWrap = document.getElementById('centralMarkerWrap');
      if (centralWrap) {{
        if (!cityName || cityName === 'JAKARTA PUSAT') {{
          centralWrap.style.opacity = '1';
        }} else {{
          centralWrap.style.opacity = '0.20';
        }}
      }}
    ...
    async function exportPoster() {{
      ...
      html2canvas(poster, {{
        ...
        onclone: function(clonedDoc) {{
          ...
          // 3. Ensure cloned Central Jakarta callout line height matches computed height
          const clonedLine = clonedDoc.getElementById('centralLineDom');
          const origLine = document.getElementById('centralLineDom');
          if (clonedLine && origLine) {{
            clonedLine.style.height = origLine.style.height;
          }}
        }}
      }}).then(canvas => {{
